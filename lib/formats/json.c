...
static void rm_fmt_foot(_UNUSED RmSession *session, RmFmtHandler *parent, FILE *out) {
    ...
    if(rm_fmt_get_config_value(session->formats, "json", "no_footer")) {
        ...
        rm_fmt_json_open(self, out);
        {
            ...
            rm_fmt_json_sep(self, out);
            rm_fmt_json_key_int(out, "total_files", session->counters->total_files);
            rm_fmt_json_sep(self, out);
            rm_fmt_json_key_int(out, "ignored_files", session->counters->ignored_files);
            rm_fmt_json_sep(self, out);
            rm_fmt_json_key_int(out, "ignored_folders",
                                session->counters->ignored_folders);
            rm_fmt_json_sep(self, out);
            rm_fmt_json_key_int(out, "duplicates", session->counters->dup_counter);
            rm_fmt_json_sep(self, out);
...
static void rm_fmt_elem(RmSession *session, _UNUSED RmFmtHandler *parent, FILE *out,
    ...
    rm_fmt_json_open(self, out);
    {
        ...
        rm_fmt_json_key_int(
            out, "progress",
            CLAMP(100 -
                      100 * ((gdouble)session->counters->shred_bytes_remaining /
